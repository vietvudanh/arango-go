...
func main() {
	...
	// vertices
	ctxAwait := driver.WithWaitForSync(context.Background())
	wp := workerpool.New(4)
	...
	for v, _ := range vertices {
		...
		if len(buffer) == 10000 {
			...
			wp.Submit(func() {
				...
				collection.CreateDocuments(ctxAwait, e)
			})
	...
	collection.CreateDocuments(ctxAwait, buffer)
	wp.StopWait()
	...
	for i, edge := range edges {
		...
		if len(bufferEdges) == 10000 {
			...
			wp.Submit(func() {
				...
				edgeCollection.CreateDocuments(ctxAwait, e)
			})
			bufferEdges = []map[string]string{}
		}
	}
	log.Println("insert edges out::", len(bufferEdges))
	edgeCollection.CreateDocuments(ctxAwait, bufferEdges)
	wp.StopWait()
